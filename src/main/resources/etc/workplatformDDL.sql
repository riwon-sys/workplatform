...
CREATE TABLE member (
    ...
    mphone VARCHAR(13) NOT NULL UNIQUE,          # not null unique : 전화번호, 고유값 설정
    memail VARCHAR(100),                                          # not null : 이메일
    mtype INT DEFAULT 0,                                            # default 0 : 상태 (0: 활동, 1: 부재, 2: 외부업무, 3: 퇴사)
    mrank VARCHAR(10) NOT NULL,                           # not null 직급 : 사원 - 대리 - 팀장 - 부서장
